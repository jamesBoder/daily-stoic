...
// The read-modify-write of the session runs inside a transaction with the
// session row locked (SELECT ... FOR UPDATE), so concurrent guess submissions
// for the same user+puzzle serialize instead of racing on a lost update.
func (s *ConfluenceService) SubmitGuess(userID, puzzleID uint, cardIDs []uint) (*GuessResult, error) {
	...
	var unlockSession *models.ConfluenceGameSession

	err = s.repo.WithTx(func(txRepo *repository.ConfluenceRepository) error {
		session, err := txRepo.GetOrCreateSessionForUpdate(userID, puzzleID)
		if err != nil {
			return err
		}
		...
		return txRepo.SaveSession(session)
	})
	if err != nil {
		return nil, err
	}
	...
	if unlockSession != nil {
		go func() {
			...
			s.library.UnlockConfluenceCards(userID, unlockSession, puzzle)
		}()
	}
